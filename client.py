...
class NewsAggregatorApp:
    def __init__(self, root):
        ...
        # Create query frame
        self.query_frame = ttk.Frame(self.main_container, style="Custom.TFrame")
        self.query_frame.pack(fill="both", expand=True)

        # Title with icon
        self.title_label = ttk.Label(
            self.query_frame, text="📰 News Content Aggregator", 
            style="Title.TLabel",
        ...
        self.articles_frame = ttk.Frame(self.main_container, style="Custom.TFrame")

        # Articles title with icon
        self.articles_title = ttk.Label(
            self.articles_frame, text="📋 Fetched Articles", style="Title.TLabel"
        )
        ...
        self.articles_text.tag_configure(
            "link", foreground=self.link_color, underline=1, font=("Helvetica", 12)
        )
        self.articles_text.tag_configure(
            "title", font=("Helvetica", 13, "bold"), foreground=self.accent_color
        )
        ...
        self.loading_label = ttk.Label(
            self.query_frame, text="", style="Subtitle.TLabel"
        )
    ...
    def submit_domains(self):
        ...
        threading.Thread(
            target=self.fetch_articles, args=(domains,), daemon=True
        ).start()
    ...
    def show_articles(self, articles):
        ...
        for idx, (title, link) in enumerate(articles, start=1):
            ...
            if link and link != "No link available":
                ...
                self.articles_text.insert(tk.END, f"    🔗 Link: {link}\n", "link")
            else:
